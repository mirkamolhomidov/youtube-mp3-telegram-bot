...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('select_')) {
        ...
        const prevMessage = userStates[chatId].messageText;
        const prevMarkup = JSON.stringify(userStates[chatId].replyMarkup);
        const newMarkup = [
            numButtons.slice(0, 5),
            numButtons.slice(5, 10),
            controlButtons
        ];
        if (prevMessage !== messageText || prevMarkup !== JSON.stringify(newMarkup)) {
            try {
                await bot.editMessageText(messageText, {
                    chat_id: chatId,
                    message_id: query.message.message_id,
                    reply_markup: { inline_keyboard: newMarkup }
                });
                userStates[chatId].messageText = messageText;
                userStates[chatId].replyMarkup = newMarkup;
            } catch (err) {
                console.error('Xato:', err);
            }
        } else {
            console.log('Xabar va tugmalar o‘zgarmagan.');
        }
